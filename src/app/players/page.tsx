        ...
        <div className="flex-1 min-w-0 space-y-1">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-1 min-w-0">
              ...
              {p.openToPlay && <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse shrink-0"/>}
              {p.lookingForPartner && <Users size={9} className="text-violet-400 shrink-0"/>}
              {isFriend && <UserCheck size={9} className="text-emerald-400 shrink-0"/>}
              {isIncoming && !isFriend && <Bell size={9} className="text-amber-400 shrink-0"/>}
            </div>
...
function RankRow({ player: p, rank, isMe, isFriend, sortKey }: {
  ...
  const statLabel = sortKey === 'winRate' ? `${wr}% WR` : sortKey === 'wins' ? `${p.stats.wins}W` : sortKey === 'matches' ? `${p.stats.totalMatches} played` : p.mmr.toLocaleString();
  const subLabel  = sortKey === 'mmr' ? 'MMR' : '';
  return (
