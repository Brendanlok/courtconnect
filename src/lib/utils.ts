...
// Builds a minimal .ics file and triggers a browser download — so a
// registered player can drop the event straight into their phone calendar.
...
export function downloadIcs(e: { id: string; summary: string; date: string; time?: string; location: string }): void {
  ...
  a.href = url; a.download = `${e.summary.replace(/[^a-z0-9]+/gi, '-')}.ics`;
  a.click();
  URL.revokeObjectURL(url);
}

export function downloadTournamentIcs(t: { id: string; name: string; date: string; time?: string; venue: string; state: string }): void {
  downloadIcs({ id: t.id, summary: t.name, date: t.date, time: t.time, location: `${t.venue}, ${t.state}` });
}
