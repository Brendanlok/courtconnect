  ...
  if (saved) {
    return (
      <div className="fixed inset-0 z-50 bg-black/75 flex items-center justify-center p-4">
        <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl p-12 text-center">
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-4 max-h-[72vh] overflow-y-auto">
          ...
          <div className="border-t border-slate-800 pt-4 space-y-3">
            <p className="text-xs text-red-500 uppercase tracking-wide font-semibold">Danger Zone</p>

            {deleteStep === 'idle' && (
              <button onClick={() => setDeleteStep('warn')}
                className="w-full flex items-center justify-center gap-2 py-2.5 border border-red-500/30 text-red-400 hover:bg-red-500/10 rounded-xl text-sm font-semibold transition-colors">
                <Trash2 size={14}/> Delete Profile
              </button>
            )}

            {deleteStep === 'warn' && (
              <div className="bg-red-500/8 border border-red-500/30 rounded-xl p-4 space-y-3">
                <div className="flex items-start gap-2.5">
                  <AlertTriangle size={16} className="text-red-400 shrink-0 mt-0.5" />
                  <div>
                    <p className="text-sm font-semibold text-red-300">This will permanently delete your profile</p>
                    <p className="text-xs text-slate-400 mt-1">All your match history, MMR, and stats will be lost. This cannot be undone.</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => setDeleteStep('idle')}
                    className="flex-1 py-2 bg-slate-800 hover:bg-slate-700 rounded-xl text-sm font-semibold transition-colors">
                    Cancel
                  </button>
            ...
            {deleteStep === 'confirm' && (
              <div className="bg-red-500/8 border border-red-500/40 rounded-xl p-4 space-y-3">
                <p className="text-sm font-semibold text-red-300">Final confirmation</p>
                <p className="text-xs text-slate-400">Type <span className="font-bold text-white">DELETE</span> to confirm.</p>
                <input
                  value={deleteInput}
                  onChange={e => setDeleteInput(e.target.value)}
                  placeholder="Type DELETE here"
                  className="w-full bg-slate-800 border border-red-500/40 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-red-500 transition-colors font-mono"/>
                <div className="flex gap-2">
                  <button onClick={() => setDeleteStep('idle')}
                    className="flex-1 py-2 bg-slate-800 hover:bg-slate-700 rounded-xl text-sm font-semibold transition-colors">
                    Cancel
                  ...
                  <button
                    ...
                    className="flex-1 py-2 bg-red-600 hover:bg-red-500 disabled:opacity-30 disabled:cursor-not-allowed text-white rounded-xl text-sm font-semibold transition-colors">
                    Delete Forever
                  </button>
                </div>
              </div>
